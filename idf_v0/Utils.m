classdef Utils
    methods(Static)
        ...
        function [Yust,Ylst] = D_airfoil2(Au,Al,X)
        ...
        function V_tank = tank_volume()
            Y = linspace(0,I.Wing(1).Span/2,20);
            X0 = linspace(0,1,99);
            chord_distribution = interp1(Res_aero.Wing.Yst, Res_aero.Wing.chord, Y, 'spline');
            [Yust,Ylst] = D_airfoil2(I.Wing(1).CST_Upper,I.Wing(1).CST_Lower,X0);
            airfoil_surf = zeros(17);
            for i=1:17 %from y=0 to y=0.85
                X1 = linspace(0,chord_distribution(i),99);
                X_tank = linspace(0.15*chord_distribution(i),0.60*chord_distribution(i),99);
                Yust = interp1(X1,Yust,X_tank);
                Ylst = interp1(X1,Ylst,X_tank);
                airfoil_surf(i) = chord_distribution(i)*trapz(Yust - Ylst);
            end
            V_tank = 2*trapz(airfoil_surf);
        end
